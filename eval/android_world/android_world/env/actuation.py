...
def execute_adb_action(
  ...
  if action.action_type in ['click', 'double_tap', 'long_press']:
    ...
    # Coordinate-based gesture: if the caller supplied an explicit start (x, y)
    # and end (x2, y2), perform a literal finger drag using those exact pixel
    # coordinates instead of deriving a cardinal direction. This preserves the
    # model-predicted gesture (distance, start point) with no precision loss.
    if (
    ...
    # Coordinate-based gesture: literal finger drag from (x, y) to (x2, y2)
    # using the exact pixel coordinates, when all four are provided.
    if (
        action.x is not None
        and action.y is not None
        and action.x2 is not None
      ...
      command = adb_utils.generate_swipe_command(
          ...
          int(action.x2),
          int(action.y2),
          action.duration if action.duration else 500,
      )
      adb_utils.issue_generic_request(command, env)
      return
